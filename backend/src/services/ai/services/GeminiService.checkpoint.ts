...
export class GeminiService {
  ...
  private buildPrompt(text: string, categories: string[], priorities: string[]): string {
    return [
      ...
      '  Product  → any complaint about physical harm, health risk, injury, pain, worsened condition,',
      '             side effects, contamination, defective/expired/wrong product, food poisoning,',
      '             allergic reaction, breathing difficulty, negligence causing harm,',
      '             trainer/instructor/coach causing injury or worsening a medical condition,',
      '             exercise or therapy that caused harm, unsafe service that harmed the person.',
      '  Packaging → complaints about damaged/broken/leaking/tampered packaging, broken seal,',
      '              cracked bottle, crushed carton, missing label — where the PACKAGING is the issue.',
      '  Trade    → bulk orders, pricing inquiries, distributor/retailer/wholesale queries,',
      '             invoices, quotations, trade schemes — purely commercial/business queries.',
      '  Invalid  → ONLY use if the input is completely unrelated to any complaint (e.g. random text,',
      '             a greeting with no complaint, or a question with zero complaint context).',
      '             Do NOT use Invalid for health, injury, service, or product complaints.',
      '',
      ...
      `           health risk, safety issue, negligence causing harm, worsened medical condition,`,
      `           trainer/instructor caused injury or harm, exercise caused harm,`,
      ...
      '  0.9-1.0 → physical injury, health/safety emergency, life-threatening, worsened medical condition',
      '  0.7-0.89 → serious harm, negligence, broken/defective product, contamination, trainer-caused harm',
      ...
      '',
      'CRITICAL RULES:',
      '  1. If the complaint describes ANY physical injury, harm, pain, worsened condition, or safety risk — always assign the highest priority regardless of category.',
      '  2. Complaints about trainers, instructors, coaches, or therapists causing harm or worsening a condition → category = Product, priority = High.',
      '  3. Never classify a genuine complaint (injury, harm, health issue, service failure) as Invalid.',
    ].join('\n');
  }
